...
void 	circleSimple(t_world *w, int xCenter, int yCenter, int radius, int color)
{
	int x, y, r2;    
	...
	for (x = -radius; x <= radius; x++)
	{
		...
		draw_line(w, xCenter + x, yCenter + y, xCenter + x, yCenter - y);
	}
}
...
{
	img_clear(world);
	circleSimple(world, WIDTH / 2, HEIGHT / 2, 50, 0x00FFFFFF);
