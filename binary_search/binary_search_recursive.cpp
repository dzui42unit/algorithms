...
int	ft_binary_search(int *arr, int find, int p, int r)
{
	int q;
	...
	if (arr[q] == find)
		return (q);
	if (arr[q] > find)
		return (ft_binary_search(arr, find, p, q -1));
	if (arr[q] < find)
		return (ft_binary_search(arr, find, q + 1, r));
}
...
int	main(void)
{
	...
	if (ft_is_sorted(arr, n))
	{
		...
		position = ft_binary_search(arr, find, 0, n -1);
		if (position != -1)
